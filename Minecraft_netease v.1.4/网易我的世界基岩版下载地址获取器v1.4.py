...
class Frame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, title='网易我的世界基岩版下载地址获取器v1.4(By daijunhao),(github:daijunhaoMinecraft),严禁倒卖!  v1.4更新内容:更新图形化界面', size=(950, 670),name='frame',style=541072384)
    ...
    def Wget_Download_按钮被单击(self,event):
        Confirm_the_information=wx.MessageDialog(None, u"是否继续执行Wget下载?", u"确认信息", wx.YES_NO | wx.ICON_QUESTION)
    ...
    def zip_Wget_download_按钮被单击(self,event):
        Confirm_the_information = wx.MessageDialog(None, u"是否继续执行Wget下载并解压缩?", u"确认信息",wx.YES_NO | wx.ICON_QUESTION)
        if Confirm_the_information.ShowModal() == wx.ID_YES:
    ...
    def Minecrat_for_Netease_按钮被单击(self,event):
        warning = wx.MessageDialog(None, u"是否继续执行?(这可能会导致你的基岩版被删除重新下载)", u"警告",wx.YES_NO | wx.ICON_QUESTION)
